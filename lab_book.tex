...
\newexperiment{example}[实验例程1]{这是一个实验例程}
\newexperiment{example2}[实验例程2]{这是另外一个实验例程}
\newexperiment{example3}[实验例程3]{这又是另外一个实验例程}
...
这里可以输入正文，这里可以写快速排序的介绍，$n*\log{n}$后面的可以加入详细的描述，比如思路分析，算法描述，编写代码，性能评价等，对比不同线程数时程序运行的时间，性能评价要加入图表分析，可以用gunplot绘图，真是很爽啊！
\lipsum[1]
%-----------------------------------------
...
\includegraphics[scale=0.5,keepaspectratio=true]{example-image-a}
